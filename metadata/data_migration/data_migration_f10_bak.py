...
def F10_dump_sum_time_line_bak(begin_time,end_time,line_ID):
	global F10_value
	F10_value=[]
	...
	if row[0] == None:
		...
		F10_value.append(outcount_sum)
		F10_value.append(time.strftime('%Y%m%d%H%M%S'))
		return F10_value

def F10_dump_sum_time_line(begin_time,end_time):
	global F10_value
	F10_value=[]
	sql="SELECT Cam_ID,sum(incount),sum(outcount),line_ID FROM `F10_result_tbl` WHERE `begin_time` > '%s' AND `end_time` < '%s' group by line_ID;"%(begin_time,end_time)
	active_db.a_cur.execute(sql)	
	rows=active_db.a_cur.fetchall()
	#print rows,len(rows)
	if len(rows) > 0:
		for row in rows:
			Cam_ID=row[0]
			...
			F10_value.append(incount_sum)
			F10_value.append(outcount_sum)
			F10_value.append(time_stamp)
			#print F10_value
			if len(F10_value)<7:
				continue
			else:
				sql="insert into F10_result_tbl (Cam_ID,begin_time,end_time,line_ID,incount,outcount,time_stamp) values('%d','%s','%s','%d','%d','%d','%s')"%(Cam_ID,begin_time,end_time,line_ID,incount_sum,outcount_sum,time_stamp)
				#history_db.h_cur.execute("insert into F10_result_tbl values(%s,%s,%s,%s,%s,%s,%s)",F10_value)
				history_db.h_cur.execute(sql)
				history_db.h_conn.commit()
	else:
		print 'This time no valid values !!!'
...
def main():
	...
	try:
		...
		for now_time in time_list:
			...
			F10_dump_sum_time_line(begin_time,end_time)
			'''
